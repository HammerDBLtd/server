...
ATTRIBUTE_COLD ahi_status btr_sea::disable_and_lock() noexcept
{
...
ATTRIBUTE_COLD ahi_status btr_sea::disable() noexcept
{
  const ahi_status was_enabled{disable_and_lock()};
  unlock();
  return was_enabled;
}

ATTRIBUTE_COLD void btr_sea::enable(bool resize,
                                    ahi_status enable_kind) noexcept
{
  ut_ad(enable_kind > AHI_OFF);
...
ATTRIBUTE_COLD void btr_sea::resize(uint n_cells) noexcept
{
  ...
  if (was_enabled)
    enabled= alloc(n_cells) ? was_enabled : AHI_OFF;
                                       ...
                                       const mtr_t &mtr) noexcept
{
  ...
  if (ut_d(const dict_index_t *block_index=) block->index)
  {
    ...
    ut_ad(btr_search.get_enabled());
    ut_ad(index->search_info.get_enabled());
                                            ...
                                            const mtr_t &mtr) noexcept
{
  ...
  uint32_t enabled_fixed_mask, fixed, mask;
  ut_d(enabled_fixed_mask= 0);
  ut_d(fixed= 0);
  ut_d(mask= 0);
  ...
  if (!n_hash_potential)
  {
    ...
    enabled_fixed_mask= info.get_enabled_fixed_mask();
    info.get_fixed_mask(enabled_fixed_mask, fixed, mask);
...
func_exit:
  ...
  DBUG_EXECUTE_IF("index_ahi_option_debug_check",
  {
    ...
    const uint8_t enabled= info.get_enabled(enabled_fixed_mask);
  ...
  noexcept
{
  ...
  else
  {
    ...
    block->index= index;
    block->ahi_left_bytes_fields= left_bytes_fields;
    ut_ad(index->search_info.ref_count < UINT32_MAX);
