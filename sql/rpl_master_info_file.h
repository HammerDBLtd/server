...
struct Master_info_file: Info_file
{
  ...
  /*
    Optional path value (for SSL): FN_REFLEN-sized null-terminated string
    with a mariadbd option for the DEFAULT. set_default() copies the
    default into buf; has_value tracks whether buf is user-set or default.
  */
  ...
  /*
    uint32 array value. Only one of DO_DOMAIN_IDS and IGNORE_DOMAIN_IDS
    can be active at a time, so separate arrays are wasteful. Until we
    refactor this pair, this struct only references existing arrays and
    does not manage their lifetimes.
  */
  struct ID_array_value: Value
  {
    /* Array of longs. (FIXME: should be uint32.) */
    DYNAMIC_ARRAY *array;
    /*
  ...
  /*
    ...
    internally as unsigned integer milliseconds. The DEFAULT comes from
    ::master_heartbeat_period (in seconds; multiplied by 1000 here).
    Parsed from the file with my_strtod(); the maximum on-disk value is
    UINT_MAX32 milliseconds (~4294967.295 seconds).
  */
